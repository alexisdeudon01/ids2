...
def get_my_files():
    ...
    for root, dirs, files in os.walk(PROJECT_DIR):
        ...
        for f in files:
            if f.endswith(EXTENSIONS) and f not in ["run_all_dependencies.py", "graph_interactif.py", "generate_graph.py", "generate_auto_graph.py"]:
...
def execute_script(script_path):
    ...
    try:
        # Détermine l'interpréteur approprié
        ...
        # Exécution et vérification du code de retour
        result = subprocess.run(command, check=True, capture_output=True, text=True, cwd=PROJECT_DIR)
...
def main():
    ...
    all_scripts_basenames = {os.path.basename(f): f for f in all_scripts_paths} # Map basename to full path
    ...
    # Convertir les basenames en chemins complets pour l'exécution
