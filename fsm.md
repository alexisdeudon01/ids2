# FSM actuelle du projet IDS2

Source de vérité: `docs/kl.md` (diagramme canonique).  
Cette version reflète l’implémentation actuelle dans `setup.sh` et le backend.
...
- `setup.sh`  
  - Top-level: `WaitUser → StartCommand → Initializing → ComponentsStarting → SupervisorRunning`
  - Deployment FSM et Docker FSM intégrées dans le déploiement Pi.
- `webapp/backend/src/ids/app/supervisor.py`  
  - Superviseur principal + sous‑états (`SupervisorMonitoring`, `SupervisorDegraded`, etc.).
- `webapp/backend/src/ids/composants/base.py`  
  - États des composants (`CompStarting`, `CompHealthy`, `CompError`, etc.).
- `webapp/backend/src/ids/app/pipeline_status.py`  
  - États du pipeline (`PipeCollecting`, `PipeOK`, `PipeRecovering`, etc.).
