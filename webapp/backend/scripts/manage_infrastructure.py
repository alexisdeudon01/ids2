#!/usr/bin/env python3
"""
Script de gestion complète de l'infrastructure IDS.
...
Commands:
    ...
    tailscale create-key            Crée une auth key

    opensearch create-domain <name> Crée un domaine OpenSearch
    opensearch list-domains         Liste les domaines
    opensearch list-indexes <endpoint> Liste les index

    pi info                         Infos système du Pi
    pi services                     Liste les services
    pi docker-ps                    Liste les conteneurs Docker
    pi restart-service <name>       Redémarre un service
"""
...
async def tailscale_list_devices(api_key: str, tailnet: str):
    ...
    async with TailscaleManager(api_key, tailnet) as manager:
        ...
        print(f"\n📡 Tailscale Devices ({len(devices)} total)\n")
...
async def tailscale_ping(api_key: str, tailnet: str, ip_address: str):
    ...
    async with TailscaleManager(api_key, tailnet) as manager:
        ...
        if not device:
            print(f"❌ Device not found: {ip_address}")
            return

        print(f"\n📡 Pinging {device.name} ({ip_address})...")
...
async def tailscale_create_key(api_key: str, tailnet: str):
    ...
    async with TailscaleManager(api_key, tailnet) as manager:
        ...
        print(f"\n✅ Auth key created:")
        print(f"   {key}")
        print(f"\n💡 Use this key to connect new devices:")
...
def opensearch_list_domains(aws_access_key: str, aws_secret_key: str, region: str):
    ...
    print(f"\n📊 OpenSearch Domains ({len(domains)} total)\n")

    for domain_name in domains:
        status = manager.get_domain_status(domain_name)
        if status:
            processing = "🔄 Processing" if status.processing else "✅ Ready"
            print(f"  • {domain_name}")
...
def opensearch_list_indexes(
    ...
    print(f"\n📚 OpenSearch Indexes ({len(indexes)} total)\n")
    print(f"{'Name':<40} {'Health':<10} {'Docs':<12} {'Size':<12}")
    print("=" * 80)

    for idx in indexes:
        health_icon = {"green": "🟢", "yellow": "🟡", "red": "🔴"}.get(idx.health, "⚪")
...
def pi_info(host: str, user: str, ssh_key: str):
    """Affiche les infos système du Pi."""
    ...
    with RaspberryPiManager(host, user, ssh_key_path=ssh_key) as pi:
        ...
        for service_name in services:
            try:
                ...
                print(f"{service_name:<30} ❌ Error: {e}")
